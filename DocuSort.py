...
#start
    ...
    def landing_page(self):
        ...
        # Load the top image (placed above the DOCUSORT text)
        try:
            top_image_path = self.resource_path("resources/HomeHeader.png")
            top_image = Image.open(top_image_path)
            top_image = top_image.resize((300, 150))
            ...
            top_img_label.pack(pady=(0, 10))
        except Exception as e:
            print(f"Error loading top image: {e}")
            fallback_label = tk.Label(
                logo_frame, text="TOP IMAGE NOT FOUND",
                font=("Courier New", 18), fg="red", bg="#131f24"
            )
            fallback_label.pack(pady=(0, 10))

        # Load the main image (next to the DOCUSORT text)
        try:
            main_image_path = self.resource_path("resources/docusort_test.png")
            ...
            fallback_label = tk.Label(
                logo_frame, text="IMAGE NOT FOUND",
                font=("Courier New", 18), fg="red", bg="#131f24"
            )
