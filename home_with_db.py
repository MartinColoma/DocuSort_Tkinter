...
class DocuSortApp:
    ...
    def submit_receiver_info(self):
        ...
        # Gather the receiver's information into variables
        receiver_first_name = self.receiver_first_name_entry.get()
        receiver_last_name = self.receiver_last_name_entry.get()
        receiver_faculty = self.receiver_faculty_combobox.get()

        # Store receiver information in a dictionary
        receiver_info = {
        ...
        # Combine both sender and receiver information into one string (for showing popup)
        all_info = "\n".join([f"{key}: {value}" for key, value in {**sender_info, **receiver_info}.items()])

        # Check for empty fields in receiver's data
        if not all(receiver_info.values()):
            messagebox.showerror("Error", "Please fill in all fields.")
            return

        # INSERT into the database using the gathered variables
        try:
            conn = sqlite3.connect('docusortDB.db')
            cursor = conn.cursor()
            ...
            conn.commit()
            print("Data inserted successfully.")  # Debugging output
        except Exception as e:
            print(f"Error inserting data: {e}")  # Print the error message
            messagebox.showerror("Database Error", "An error occurred while saving data.")
        finally:
            conn.close()
        ...
        # After inserting, go back to landing page
