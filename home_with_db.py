...
class DocuSortApp:
    ...
    def landing_page(self):
        ...
        label = tk.Label(self.root, text="DOCUSORT", font=("Helvetica", 24))
        label.pack(pady=50)
        
        start_button = tk.Button(self.root, text="START", font=("Helvetica", 14), command=self.sender_info_page)
        start_button.pack()
    ...
    def sender_info_page(self):
        ...
        # Sender Info Page
        tk.Label(self.root, text="Sender Info", font=("Helvetica", 18)).pack(pady=20)
        ...
        tk.Label(form_frame, text="Student ID #:").grid(row=1, column=0, padx=10, pady=5)
        ...
        self.section_entry.grid(row=1, column=3, padx=10, pady=5)

        # Faculty and Course (in the same row)
        tk.Label(form_frame, text="Faculty:").grid(row=2, column=0, padx=10, pady=5)
        ...
        self.faculty_combobox.grid(row=2, column=1, padx=10, pady=5)

        tk.Label(form_frame, text="Course:").grid(row=2, column=2, padx=10, pady=5)
        self.course_combobox = ttk.Combobox(form_frame)
        self.course_combobox.grid(row=2, column=3, padx=10, pady=5)
    ...
    def receiver_info_page(self):
        ...
        # Receiver Info Page
        tk.Label(self.root, text="Receiver Info", font=("Helvetica", 18)).pack(pady=20)
        ...
        self.receiver_first_name_entry.grid(row=0, column=1, padx=10, pady=5)

        tk.Label(form_frame, text="Last Name:").grid(row=0, column=2, padx=10, pady=5)
        self.receiver_last_name_entry = tk.Entry(form_frame)
        self.receiver_last_name_entry.grid(row=0, column=3, padx=10, pady=5)
        ...
        self.receiver_faculty_combobox.grid(row=1, column=1, padx=10, pady=5)
    ...
    def submit_receiver_info(self):
        # Gather all the entered information and show it in a popup
        receiver_info = {
        ...
        # Show a popup with all entered information
        info_str = "\n".join([f"{key}: {value}" for key, value in receiver_info.items()])
        messagebox.showinfo("Submitted Information", info_str)
